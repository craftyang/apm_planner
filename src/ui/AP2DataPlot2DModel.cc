...
void AP2DataPlot2DModel::addType(QString name,QStringList types,QStringList names)
{
    QSqlQuery fmtinsertquery(m_sharedDb);
    if (!m_msgNameToInsertQuery.contains(name))
    {
        ...
        {
            typechars += types.at(i) + ((i < types.size()-1) ? "," : "");
        }
        ...
        fmtinsertquery.bindValue(":idx",m_fmtIndex++);
...
void AP2DataPlot2DModel::addRow(QString name,QList<QPair<QString,QVariant> >  values,quint64 index)
{
    ...
    {
        query.bindValue(QString(":") + values.at(i).first,values.at(i).second.toInt());
    }
    if (!query.exec())
    {
...
QString AP2DataPlot2DModel::makeCreateTableString(QString tablename, QStringList formatstr,QStringList variablestr)
{
    ...
    for (int j=0;j<variablestr.size();j++)
    {
        ...
        name = "\"" + name + "\"";
        QChar typeCode = formatstr.at(j).at(0);
